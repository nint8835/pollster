...
export const Route = createFileRoute('/(app)/polls/$pollId/manage')({
...
function RouteComponent() {
  ...
  const handleCreatePollOption = async () => {
    ...
    await queryClient.invalidateQueries(
      getPollQuery({ pathParams: { pollId } }),
    )
  }
          ...
          {poll.options.map((option) => (
            <div key={option.id} className="flex gap-2">
              <Input
                ...
                onChange={(e) =>
                  setEditingOption({ id: option.id, name: e.target.value })
                }
                onBlur={handleEditPollOption}
              />
              <Button
                onPress={() => handleDeletePollOption(option.id)}
                color="danger"
              >
                <Trash2 />
              </Button>
      ...
      <CardFooter>
        <div className="flex w-full items-center justify-between gap-2">
          <div className="italic text-zinc-400">
            Voters will be required to complete approximately{' '}
            {requiredComparisonCount.toLocaleString()}{' '}
            {pluralize(requiredComparisonCount, 'comparison', 'comparisons')}.
          </div>
          {nextPollStatus && (
            <Button
              onPress={handleNextStatus}
              {...nextPollPropsRecord[poll.status]}
            ></Button>
          )}
        </div>
      </CardFooter>
